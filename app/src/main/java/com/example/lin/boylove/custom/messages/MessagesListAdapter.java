package com.example.lin.boylove.custom.messages;
...
public class MessagesListAdapter extends RecyclerView.Adapter<ViewHolder>
    ...
    * */
    ...
    public void addToStart(ChatMessage message, boolean scroll) {
        ...
        Wrapper<ChatMessage> element = new Wrapper<>(message);
        items.add(0, element);
     ...
     */
    public void addToEnd(List<ChatMessage> messages, boolean reverse) {
     ...
     */
    public void update(int oldId, ChatMessage newMessage) {
        int position = getMessagePositionById(oldId);
        if (position >= 0) {
            Wrapper<ChatMessage> element = new Wrapper<>(newMessage);
     ...
     */
    public void delete(List<ChatMessage> messages) {
    ...
    public ArrayList<ChatMessage> getSelectedMessages() {
        ...
        for (Wrapper wrapper : items) {
            if (wrapper.item instanceof IMessage && wrapper.isSelected) {
                selectedMessages.add((ChatMessage) wrapper.item);
            }
        }
     ...
     */
    public String getSelectedMessagesText(Formatter<ChatMessage> formatter, boolean reverse) {
     ...
     * @return formatted text by {@link Formatter}. If it's {@code null} - {@code MESSAGE#toString()} will be used.
     */
    public String copySelectedMessagesText(Context context, Formatter<ChatMessage> formatter, boolean reverse) {
        String copiedText = getSelectedText(formatter, reverse);
    ...
    public void deleteSelectedMessages() {
        List<ChatMessage> selectedMessages = getSelectedMessages();
     ...
     */
    public void setOnMessageClickListener(OnMessageClickListener<ChatMessage> onMessageClickListener) {
     ...
     */
    public void setOnMessageViewClickListener(OnMessageViewClickListener<ChatMessage> onMessageViewClickListener) {
     ...
     * @param onMessageViewClickListener click listener.
     */
    public void registerViewClickListener(int viewId, OnMessageViewClickListener<ChatMessage> onMessageViewClickListener) {
     ...
     */
    public void setOnMessageLongClickListener(OnMessageLongClickListener<ChatMessage> onMessageLongClickListener) {
     ...
     */
    public void setOnMessageViewLongClickListener(OnMessageViewLongClickListener<ChatMessage> onMessageViewLongClickListener) {
    ...
    * */
    ...
    private void generateDateHeaders(List<ChatMessage> messages) {
    ...
    private int getMessagePositionById(int id) {
        for (int i = 0; i < items.size(); i++) {
            ...
            if (wrapper.item instanceof IMessage) {
                ChatMessage message = (ChatMessage) wrapper.item;
                if (message.getId() == id) {
                    return i;
                }
            }
    ...
    private boolean isPreviousSameDate(int position, Date dateToCompare) {
        ...
        if (items.get(position).item instanceof IMessage) {
            Date previousPositionDate = ((ChatMessage) items.get(position).item).getCreatedAt();
    ...
    private boolean isPreviousSameAuthor(int id, int position) {
        ...
        if (items.size() <= prevPosition) return false;
                ...
                && ((ChatMessage) items.get(prevPosition).item).getUser().getId() == id;
    }
    ...
    private void notifyMessageClicked(ChatMessage message) {
        if (onMessageClickListener != null) {
            onMessageClickListener.onMessageClick(message);
        }
    }

    private void notifyMessageViewClicked(View view, ChatMessage message) {
        if (onMessageViewClickListener != null) {
            onMessageViewClickListener.onMessageViewClick(view, message);
        }
    }

    private void notifyMessageLongClicked(ChatMessage message) {
        if (onMessageLongClickListener != null) {
            onMessageLongClickListener.onMessageLongClick(message);
        }
    }

    private void notifyMessageViewLongClicked(View view, ChatMessage message) {
        if (onMessageViewLongClickListener != null) {
            onMessageViewLongClickListener.onMessageViewLongClick(view, message);
        }
    }

    private View.OnClickListener getMessageClickListener(final Wrapper<ChatMessage> wrapper) {
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (selectionListener != null && isSelectionModeEnabled) {
                    ...
                    ChatMessage message = (wrapper.item);
    ...
    private View.OnLongClickListener getMessageLongClickListener(final Wrapper<ChatMessage> wrapper) {
    ...
    private String getSelectedText(Formatter<ChatMessage> formatter, boolean reverse) {
        StringBuilder builder = new StringBuilder();

        ArrayList<ChatMessage> selectedMessages = getSelectedMessages();
